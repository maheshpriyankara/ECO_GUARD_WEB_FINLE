...
namespace bbq
{
   ...
   WebConfigurationManager.ConnectionStrings["conn"].ConnectionString);
        ...
        public static string employeeSelected(string empName, string year, string month)
        {
            ...
            if (result_epfNo.Equals("") || result_epfNo.Equals("0"))
            {
                conn.Open();
                reader = new SqlCommand("select a.*,b.name,c.empid,c.line from paysheet as a,emp as b,empbackup as c where a.month_2='" + year + "/" + month + "' and a.empID_1=b.id and b.empid='" + result_empNo + "' and b.empid=c.empid  and c.month='" + year + "/" + month + "'", conn).ExecuteReader();
                if (reader.Read())
                {
