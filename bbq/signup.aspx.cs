  ...
  WebConfigurationManager.ConnectionStrings["conn"].ConnectionString);
        ...
        protected void Button1_Click3(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                {
                    con.Open();
                    reader = new SqlCommand("select otp,requset_date from otp_request where empid='" + Session["userID"] + "' and mobile='" + Session["mobile"] + "' order by id desc", con).ExecuteReader();
                    if (reader.Read())
                    {
                        otp_ = reader[0] + "";
                        if (reader.GetString(0).Equals(text_otp.Value))
                        {
                            if ((db.getSysDateTime() - reader.GetDateTime(1)).Hours > 24)
                            {
                                Response.Write("<script>alert('Sorry OTP has Expired.Please Request again')</script>");
                                text_otp.Value = "";

                            }
                            else
                            {
                                Response.Redirect("download_user.aspx");
                            }
                        }
                        else
                        {
                            Response.Write("<script>alert('" + "Sorry OTP is not Matching. Please try again" + "')</script>");
                        }
                    ...
                    {
                        Response.Write("<script>alert('Sorry OTP not Found. Please Request again')</script>");
                    }
                    con.Close();
                }
                else if (Session["userType"] != null && Session["userType"].ToString().Equals("Admin"))
                {
                    con.Open();
                    reader = new SqlCommand("select otp,requset_date from otp_request where empid='" + Session["userID"] + "' and mobile='" + Session["mobile"] + "' order by requset_date desc", con).ExecuteReader();
                    if (reader.Read())
                    {
                        if (reader.GetString(0).Equals(text_otp.Value))
                        {
                ...
                else
                {
                    if (text_nic.Value.ToString().Equals(""))
                    {
                        Response.Write("<script>alert('Sorry Please Input Your NIC')</script>");
                    }
                    else
                    {
                        try
                        {
                            con.Open();
                            reader = new SqlCommand("select password from user_admin where username='" + text_nic.Value + "'", con).ExecuteReader();
                            if (reader.Read())
                            {
                                if (reader.GetString(0).Equals(text_otp.Value))
                                {
                                ...
                                {
                                    Response.Write("<script>alert('Sorry Invalied Password for User')</script>");
                                }
                            ...
                            else
                            {
                                con.Close();
                                con.Open();
                                reader = new SqlCommand("select nic,mobilenumber,title,firstname,id from emp where nic='" + text_nic.Value + "'", con).ExecuteReader();

                                if (reader.Read())
                                {
                                    Session["userID"] = reader[4] + "";
                                    con2.Open();
                                    reader2 = new SqlCommand("select * from user_Admin where empid='" + Session["userID"] + "'", con2).ExecuteReader();
                                    if (reader2.Read())
                                    {
                                ...
                                else
                                {
                                    ...
                                    Response.Write("<script>alert('Sorry Invalied NIC')</script>");
                                }
                                con.Close();
                            }
                        }
