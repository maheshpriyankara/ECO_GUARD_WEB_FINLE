...
namespace bbq
{
    public class db
    {
        ...
        public string getCurrentSalaryPeriod(SqlConnection conn, SqlDataReader reader)
        {
            ...
            {
                conn.Open();
                reader = new SqlCommand("select month from empbackup where lock='true' order by id desc", conn).ExecuteReader();
                if (reader.Read())
                {
    ...
    WebConfigurationManager.ConnectionStrings["conn"].ConnectionString);
            ...
            try
            {
                ...
                conn.Open();
                reader = new SqlCommand("select id,empid from emp where id='" + epfno + "'", conn).ExecuteReader();
                if (reader.Read())
                {
                    system_id = reader[0] + "";
                    empid = reader[1] + "";
