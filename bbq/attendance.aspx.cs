...
namespace bbq
{
   ...
   WebConfigurationManager.ConnectionStrings["conn"].ConnectionString);
        ...
        public static string employeeSelected(string empName, string year, string month)
        {
            ...
            {
                result_epfNo = result.Split(',')[1].ToString().Split('-')[1];
                reader = new SqlCommand("select a.*,b.name,c.empid,c.line from paysheet as a,emp as b,empbackup as c where a.month_2='" + year + "/" + month + "' and a.empID_1=b.id and b.empid='" + result_epfNo + "' and b.empid=c.empid  and c.month='" + year + "/" + month + "'", conn).ExecuteReader();
                if (reader.Read())
                {
                    temp = "" + "_" + reader[52].ToString() + "_" + reader[51].ToString() + "_" + reader[53].ToString() + "_" + reader.GetDouble(7) + " Days" + "_" + reader.GetDouble(8) + " Days";
                }
            }
            else
            {
        ...
        protected async void Next_Click1Async(object sender, EventArgs e)
        {
            ...
            {
                var response = await client.GetAsync("http://geryjdakdai-001-site12.atempurl.com/api/home/getall?empname=47&year=2023&month=March");
            ...
            else
            {
                try
                {
                    ...
                    if (epfNo==0)
                    {
                        try
                        {
                            epfNo = Int32.Parse(text_attendanceId.Value);
                        }
                        catch (Exception)
                        {
                        }
                        conn.Open();
                        reader = new SqlCommand("select id from emp where empid='" + epfNo + "'", conn).ExecuteReader();
                        if (reader.Read())
                        {
                            idL = reader[0] + "";
                        }
