...
WebConfigurationManager.ConnectionStrings["conn"].ConnectionString);
   ...
   WebConfigurationManager.ConnectionStrings["conn"].ConnectionString);
        ...
        public static string employeeSelected(string empName, string year, string month)
        {
            ...
            {
                var result_attid= result_epfNo = result.Split(',')[1].ToString().Split('-')[1];
                conn.Open();
                reader = new SqlCommand("select a.*,b.name,c.empid,c.line from paysheet as a,emp as b,empbackup as c where a.month_2='" + year + "/" + month + "' and a.empID_1=b.id and b.empid='" + result_attid + "' and b.empid=c.empid  and c.month='" + year + "/" + month + "'", conn).ExecuteReader();
                if (reader.Read())
                {
                    id = reader[1] + "";
                    temp ="_" + reader[52].ToString() + "_" + reader[51].ToString() + "_" + reader[53].ToString();
                }
                conn.Close();
